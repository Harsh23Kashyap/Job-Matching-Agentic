...
- [ ] OpenAPI version is **1.1.0**: `curl -s http://localhost:8001/openapi.json | python3 -c "import sys,json; print(json.load(sys.stdin)['info']['version'])"`
- [ ] Tests green: `cd backend && pytest ../tests -q` (expect **69 passed**)
