...
\midrule
Portal-default composite (6-channel) & \texttt{composite\_eval\_report.json} (weights 28/27/10/15/10/10) & \textbf{0.949} \\
Multimodal weighted blend & \texttt{comparison\_table.json} & \textbf{0.924} \\
RRF ensemble (4 lists) & \texttt{comparison\_table.json} & \textbf{0.913} \\
Learned fusion (logistic, held-out) & \texttt{table11\_fusion.json} & \textbf{0.917} \\
TF--IDF / BM25 lexical baselines & \texttt{comparison\_table.json} & \textbf{0.905 / 0.902} \\
\bottomrule
\end{tabular}
\end{center}
...
Reported numbers (portal composite nDCG@5 \textbf{0.949}, with ranking \textbf{parity} --- no method statistically distinguishable at $n{=}30$, fails Holm; held-out ECE \textbf{0.019}; cross-encoder $\Delta$nDCG@5 = \textbf{$-$0.108} when reranking a pool; hard negatives \textbf{150} pairs with \textbf{0} conflicts; fairness audit \textbf{7/10} flagged) are drawn from named artifacts cited in the paper tables.
