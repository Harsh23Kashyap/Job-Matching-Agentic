...
\noindent\textbf{Corresponding author:} First Author (\url{author@example.edu})
...
Full composite ablation & \texttt{ablation\_summary.json} (40/30/15/10/5) & \textbf{0.942} \\
...
\begin{itemize}[noitemsep]
  ...
  \item Synthetic fairness audit (10 fabricated pairs): \textbf{6/10} flagged; engineering sanity check only, not demographic fairness validation in live hiring.
\end{itemize}
...
Source code for the platform, benchmark drivers, and regression tests is available in the \textbf{JobMatch project repository at the submission tag}. Key entry points: \texttt{backend/gateway/}, \texttt{backend/agents/}, \texttt{frontend/src/}, and \texttt{tests/benchmarks/test\_eval\_regression.py}.
...
Reported numbers (nDCG@5 \textbf{0.969}, \textbf{0.968}, \textbf{0.942}; cross-encoder $\Delta$nDCG@5 = \textbf{$-$0.108}; hard negatives \textbf{150} pairs with \textbf{0} conflicts; fairness audit \textbf{6/10} flagged) are drawn from named artifacts cited in the paper tables.

\vspace{0.5em}
\noindent\small\textit{Aligned with \texttt{docs/submission/jaamas/manuscript/main.tex} (May 2026). Author metadata pending final submission.}
