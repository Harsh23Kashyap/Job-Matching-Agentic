...
We evaluated \emph{résumé-to-job matching}: for each of 30 sample résumés, the system ranks 15 job postings and is judged at cutoff $K{=}5$.
The 47 manually labeled pairs use the relevance scale defined in Section~\ref{sec:5-corpus}.
Metrics are macro-averaged across all 30 résumés so that no single profile dominates the score.
...
When a candidate edits a résumé or an employer revises a job description, the owning agent rebuilds a versioned snapshot and shared embedding, then notifies the Matchmaking layer through the event bus (Section~\ref{sec:3}--Section~\ref{sec:4}).
...
Lightweight scorers complete in well under one millisecond per résumé on 15 jobs; the full composite bi-encoder run averages ${\sim}0.4$\,ms per query.
