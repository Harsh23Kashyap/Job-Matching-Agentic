...
|---|----------------|--------|----------------------|
| **1.1** | Remove all `\todo{}` markers (28 total) | ✅ DONE | 0 remain. §2 citations (11): cited `Wooldridge2009`, `Reimers2019`+`Manning2008`, `Ribeiro2016` where they fit; softened the other 8 (no fabricated refs). §4.6 phase11, §5.2 calibration/feedback resolved with real numbers. |
...
| # | Review comment | Status | Where / how resolved |
|---|----------------|--------|----------------------|
| **2.5** | Reduce repetition across §3 | ✅ DONE | `section-3-architecture.tex` — trimmed repeated read-only/ownership statements. |
| **2.6** | Compress §3.4 | ✅ DONE | Restructured into 4 subsubsections: Ownership and state · Shared state · Event communication · Human control. |
| **2.7** | Match-scoring figure before §5.2 | ✅ DONE | `section-5-quality-metrics.tex` — forward-reference to the scoring-flow figure (Fig 8) before Eq. 6. |
...
| Item | Status | Result |
|------|--------|--------|
| Composite ablation under portal weights (28/27/10/15/10/10) | ✅ DONE | full composite nDCG@5 = **0.949** (`tab-ablation.tex`, §6, abstract) |
| Statistical significance | ✅ DONE | composite vs semantic **p=0.048**; multimodal/RRF p=0.010/0.009 (§6) |
...
2. **Cross-encoder re-run** — the `ms-marco` model isn't cached and the network is blocked here, so the committed CE numbers (0.834, −0.108) stand; §6 was worded to avoid implying a stale baseline. Re-run only where the CE model/network is available (optional; was never a review TODO).
